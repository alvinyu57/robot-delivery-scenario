...
public:
  ...
  : Node("traffic_police_node")
  {
    ...
    if (speed_limit_ < 0.0) {
      throw std::invalid_argument("speed_limit must not be negative");
    }

    violation_publisher_ =
      create_publisher<delivery_robot_interfaces::msg::SpeedViolation>(kViolationTopic, 10);
...
private:
    ...
    const delivery_robot_interfaces::msg::RobotState::ConstSharedPtr message)
  {
    ...
    RCLCPP_WARN(
      get_logger(), "Robot '%s' exceeded the speed limit: %.2f m/s > %.2f m/s%s",
